...
export default function BigClockOptions(props: BigClockOptionsProps) {
  ...
  const handleClickEvent = (ev: MouseEvent) : void => {
    // if the click happens outside of the component
    // close box with setToggleVisible
    if (ev.target instanceof HTMLElement && !detailsRef.current?.contains(ev.target)) {
  ...
  return (
    <>
      <div className='toggleButton'>
        <button onClick={toggle} className={toggleVisible ? 'active' : ''}>
      ...
      <div className={`toggleContainer ${toggleVisible ? 'active' : ''}`}>
        <div>
